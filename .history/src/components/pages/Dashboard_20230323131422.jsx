...
const Dashboard = () => {
  ...
  const filterData = async (data) => {
    try{
      const { data } = await axios.get(`http://ec2-15-206-210-177.ap-south-1.compute.amazonaws.com:2000/adminroute/filter?filter=${data}`)
      console.log(data)
    }catch(e){
      console.log(e)
    }
...
<div className="dropdown">
        ...
        <div className="dropdown-content">
          <p onClick={() =>filterData('week') } > Weakly</p>
          <p onClick={() =>filterData('month') } > Monthly</p>
          <p onClick={() =>filterData('yearly') } > Yearly</p>
        </div>
      </div>

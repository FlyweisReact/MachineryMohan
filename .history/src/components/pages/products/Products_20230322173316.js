...
const Products = () => {
  ...
  const SingleProduct = async () => {
    try {
      const { data } = await axios.get(
        `http://ec2-15-206-210-177.ap-south-1.compute.amazonaws.com:2000/product/get/${id}`
      );
      setSingle(data);
    } catch (err) {
      console.log(err);
  ...
  const deleteData = async (id) => {
    try {
      const { data } = await axios.delete(
        `http://ec2-15-206-210-177.ap-south-1.compute.amazonaws.com:2000/product/delete/${id}`
      );
      console.log(data);
  ...
  function DetailModal(props) {
    ...
    return (
      ...
      >
        <Modal.Header closeButton>
          <Modal.Title id="contained-modal-title-vcenter">  {single?.Product_name} </Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <Container>
          <img src={single?.image} alt='' style={{width : '200px' , display : 'block' , margin : 'auto'}} />
          <div className="cont">
          <p className="cont-p">
              {single?.Product_name}
            </p>

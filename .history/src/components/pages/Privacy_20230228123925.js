...
const Privacy = () => {
    ...
    function MyVerticallyCenteredModal(props) {
      ...
      const postData = async (e) => {
        e.preventDefault()
        try{
            const { data } = await axios.post("http://ec2-15-206-210-177.ap-south-1.compute.amazonaws.com:2000/privacy/" , {privacy : price } )
            console.log(data)
            props.onHide()
    ...
    const deleteData = async (id) => {
        try{
                const { data } = await axios.delete(`http://ec2-15-206-210-177.ap-south-1.compute.amazonaws.com:2000/privacy/${id}`)
                console.log(data)
          ...
          {data?.privacy?.map((i , index) => (
            <tr key={index}>
              <td>
                ...
                <td>
                  <AiFillDelete color="blue" onClick={() => deleteData(i._id)} />
                </td>
              </tr>
          ))}
          </tbody>

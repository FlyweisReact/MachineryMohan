...
const Inventory = () => {
  ...
  function MyVerticallyCenteredModal(props) {
    ...
    const putHandler = async (e) => {
      ...
      try {
        const data = await axios.post(
          `http://ec2-15-206-210-177.ap-south-1.compute.amazonaws.com:2000/category/edit/category/${id}`,
          { image: imageUrl, category }
        );
        console.log(data);
  ...
  function DetailModal(props) {
    ...
    const fetchSingleCategory =useCallback(async() => {
      try {
        const { data } = await axios.get(
          `http://ec2-15-206-210-177.ap-south-1.compute.amazonaws.com:2000/category/getone/${id}`
        );
        setEachData(data);
      } catch (e) {
        console.log(e);
